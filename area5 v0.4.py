...
def upload_file():
    global img # for now you can only upload images for testing
    f_types = [('Jpg Files', '*.jpg')]
    filename = filedialog.askopenfilename(filetypes=f_types)
    img = ImageTk.PhotoImage(file=filename)
    filebtn = Button(win,image=img) 
...
filebtn = Button(area3, text='Upload File', width=20,command = lambda:upload_file())
filebtn.place(x=600, y=600) 
